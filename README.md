...
| POST   | /api/accounts     | Creates an account using the information sent inside the `request body`.                                   |
| GET    | /api/accounts     | Returns an array of accounts.                                                                                |
| GET    | /api/accounts/:id | Returns the account object with the specified `id`.                                                       |
| DELETE | /api/accounts/:id | Removes the account with the specified `id` and returns the deleted account.                                 |
| PUT    | /api/accounts/:id | Updates the account with the specified `id` using data from the `request body`. Returns the modified account |
